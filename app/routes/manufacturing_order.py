...
async def complete_manufacturing_order(
    ...
    try:
        ...
        if not result.can_complete and not result.success:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=ErrorResponse(error="BadRequest", message="Cannot complete manufacturing order due to shortages or other issues.", details=result.shortages).model_dump())
        return result
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=ErrorResponse(error="ValueError", message=str(e)).model_dump())
    except Exception as e:
